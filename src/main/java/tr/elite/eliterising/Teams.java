...
public class Teams {
    ...
    public static void addPlayer(Player p, String teamName) {
        ...
        String customName = COLORS.reverseValue(teamName) + "[" + teamName + "] " + ChatColor.GOLD + p.getName();
    ...
    public static HashMap<Player,String> getMap() {
     ...
     public static ChatColor getTeamColor(String team) {
        return COLORS.reverseValue(team);
     }
    ...
    public static boolean exitsTeam(String team) {
    ...
    public static void distributeTeams(int n) {
        ArrayList<Player> allPlayers = getPlayers();
        ArrayList<Player> teamedPlayers = new ArrayList<>();
        for (int i = 0;i < allPlayers.size() / n;i++) {
            // Baş Oyuncular
            addPlayer(getPlayers().get(i),COLORS.getValue(getColors(true).get(i)));

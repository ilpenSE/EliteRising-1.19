...
import tr.elite.eliterising.Invite;
...
public class CommandTeam implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!equalsOneMore(operator,"list","create","leave","invite","message","msg","invites")) {
        ...
        if (operator.equalsIgnoreCase("create")) {
            ...
            sendMessage(ChatColor.RED + LANGUAGE.getValue("eliterising.cmd.team.leaveSuc"),player);
            ...
            Player toInvitePlayer = getPlayer(args[1]);
            if (toInvitePlayer == null) {
                sendError(LANGUAGE.getValue("eliterising.cmd.team.invite404Player"),player);
                return true;
            }
            String code = createCode(player.getName(),toInvitePlayer.getName());

            registerInvite(code);
            getScheduler().scheduleSyncDelayedTask(instance, () -> {
                expire(code);
                if (!existInvite(code)) {
                    sendMessage(LANGUAGE.getValue("eliterising.cmd.team.invite.expired").replace("$",toInvitePlayer.getName()),player);
                }
            },20L * ((long) expireTime));

            dispatchCommand(getConsoleSender(),"tellraw " + toInvitePlayer.getName() + " {\"text\":\"[\",\"color\":\"gold\",\"extra\":[{\"text\":\"EliteRising\",\"color\":\"dark_red\"},{\"text\":\"]\",\"color\":\"gold\"},{\"text\":\" "+player.getName()+" \",\"color\":\"red\"},{\"text\":\""+LANGUAGE.getValue("eliterising.cmd.team.invite.JSON.generic")+"\",\"color\":\"yellow\"},{\"text\":\"\\n\"},{\"text\":\""+LANGUAGE.getValue("eliterising.cmd.team.invite.JSON.accept")+"\",\"color\":\"green\",\"clickEvent\":{\"action\":\"run_command\",\"value\":\"/eldavet accept "+code+"\"},\"hoverEvent\":{\"action\":\"show_text\",\"value\":\""+LANGUAGE.getValue("eliterising.cmd.team.invite.JSON.acceptHover")+"\"}},{\"text\":\" \"},{\"text\":\""+LANGUAGE.getValue("eliterising.cmd.team.invite.JSON.reject")+"\",\"color\":\"red\",\"clickEvent\":{\"action\":\"run_command\",\"value\":\"/eldavet reject "+ code +"\"},\"hoverEvent\":{\"action\":\"show_text\",\"value\":\""+LANGUAGE.getValue("eliterising.cmd.team.invite.JSON.rejectHover")+"\"}}]}");
            ...
            sendMessage(LANGUAGE.getValue("eliterising.cmd.list.allTeams") + ": " + list + ChatColor.RESET +
                    "\n" + LANGUAGE.getValue("eliterising.cmd.list.urTeam") + ": " + ((!hasTeam(player)) ? LANGUAGE.getValue("eliterising.cmd.list.no") : getTeamColor(getTeam(player)) + getTeam(player)),player);
        } else if (operator.equalsIgnoreCase("invites")) {
            ArrayList<String> codes = getInvites();
            if (codes.size() == 0) {
                sendMessage(LANGUAGE.getValue("eliterising.cmd.invites.mines") + ": " + LANGUAGE.getValue("eliterising.cmd.list.no"),player);
                return true;
            }
            String list = "";
            for (String code : codes) {
                list += ChatColor.YELLOW + LANGUAGE.getValue("eliterising.cmd.invites.receiver") + ": " + ChatColor.GOLD + getInvited(code).getName() + ChatColor.AQUA + " = " + code + ChatColor.YELLOW + (codes.indexOf(code) == codes.size() - 1 ? "" : "\n");
            }
            sendMessage(LANGUAGE.getValue("eliterising.cmd.invites.mines") + ":\n" + list,player);
        }
        return true;
    }
    public static int expireTime = instance.getConfig().getInt("inviteExpire",60);
}

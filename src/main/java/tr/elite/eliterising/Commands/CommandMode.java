    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (operation) {
            ...
            case "edit":
                ...
                int i = 0;
                for (int j = 0; j < CUSTOM_GAME_MODES.size(); j++) {
                    HashMap<String, ArrayList<String>> h1 = CUSTOM_GAME_MODES.get(j);
                    if (h1.containsKey(modeName1)) {
                        i = j;
                    }
                }
                CUSTOM_GAME_MODES.remove(i);

                HashMap<String, ArrayList<String>> h3 = new HashMap<>();
                h3.put(modeName1, items1);

                CUSTOM_GAME_MODES.add(h3);
            ...
            case "delete":
                ...
                int i1 = 0;
                for (int j1 = 0; j1 < CUSTOM_GAME_MODES.size(); j1++) {
                    HashMap<String, ArrayList<String>> h4 = CUSTOM_GAME_MODES.get(j1);
                    if (h4.containsKey(modeName2)) {
                        i1 = j1;
                    }
                }
                CUSTOM_GAME_MODES.remove(i1);
                sendMessage(LANGUAGE.getValue("eliterising.cmd.mode.removeSuc"), player);
                break;
            case "list":
                String s = "";
                for (int i2 = 0;i2 < CUSTOM_GAME_MODES.size();i2++) {
                    HashMap<String,ArrayList<String>> hash = CUSTOM_GAME_MODES.get(i2);
                    s += "" + ChatColor.GOLD + removeFirstLastValues(String.valueOf(Collections.singletonList(hash.keySet()).get(0))) + (i2 == CUSTOM_GAME_MODES.size() - 1 ? "" : (ChatColor.AQUA + ", "));
                }
                sendMessage(

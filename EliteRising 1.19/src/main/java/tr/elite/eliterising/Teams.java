...
public class Teams {
    ...
    public static void addPlayer(Player p, String teamName) {
        ...
        String customName = new Language().toEN(teamName) + "[" + teamName + "] " + ChatColor.GOLD + p.getName();
    ...
    public static void distributeTeams(TeamModes teamMode) {
        ...
        for (int i = 0;i < allPlayers.size() / teamMode.getNumber();i++) {
            // Baş Oyuncular
            addPlayer(getPlayers().get(i),new Language().getEN(getColors(true).get(i).name()));
